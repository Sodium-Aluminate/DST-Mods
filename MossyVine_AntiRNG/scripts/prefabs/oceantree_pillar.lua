...
local OceanCodeList = {}
local function testOcean(groundCode)
	if isEmptyTable(OceanCodeList) then -- 把 OCEAN 开头的地皮抄下来
		if GROUND then
			for k,v in pairs(GROUND) do
				if (string.sub(k,1,6) == "OCEAN_") then
					OceanCodeList[k] = v
				end
			end
	...
	for k,v in pairs(OceanCodeList) do
		if (groundCode == v) then
			return true
		end
	end
	return false
...
local function spawnvine(inst)
	print("mossy vine anti rng: hooked spawnvine fn")
	-- spawn vine
	...
	-- copy the values
	local MAX = TUNING.SHADE_CANOPY_RANGE_SMALL + TUNING.WATERTREE_PILLAR_CANOPY_BUFFER -- 22 + 1 = 23
	local NEW_VINES_SPAWN_RADIUS_MIN = 6
	local OCEANVINES_FAMILY_RANGE = 4 -- 我们期望无花果能不要距离其他无花果太远,以一个地皮为界线。
	local VINE_TAGS = {"oceanvine"}
	-- random pos
	for i = 1, 21, 1 do 
		print("mossy vine anti rng: rolling: ", i)
		local offset = NEW_VINES_SPAWN_RADIUS_MIN + math.random() * (MAX - NEW_VINES_SPAWN_RADIUS_MIN)
		local theta = math.random() * PI * 2
			
		local targetX = x + math.cos(theta) * offset;
		local targetY = 0;
		local targetZ = z + math.sin(theta) * offset;
		
		-- check is oceanvine nearby and not on the ocean 
		local vineNearby = TheSim:FindEntities(targetX, targetY, targetZ, OCEANVINES_FAMILY_RANGE, VINE_TAGS)
		local vineAround = TheSim:FindEntities(targetX, targetY, targetZ, MAX * 2, VINE_TAGS)
		local isOcean = TheWorld and TheWorld.Map and testOcean(TheWorld.Map:GetTileAtPoint(targetX, targetY, targetZ)) -- 没有map时回滚为nil（为什么会没有map啊）
		
		if (((#vineNearby > 0 or #vineAround == 0 or i > 15) and (not isOcean)) or i > 20) then -- 最后一次强制生成
			print("mossy vine anti rng: start spawn vine.")
			-- spwan
			vine.Transform:SetPosition(targetX, targetY, targetZ)
			vine:fall_down_fn()
